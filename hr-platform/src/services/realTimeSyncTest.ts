...
export async function testRealTimeSync() {
    ...
    try {
        ...
        try {
            ...
            const leaveRequestRef = await addDoc(collection(getFirebaseDb(), 'leaveRequests'), testLeaveRequest);
            console.log(`✅ Created leave request: ${leaveRequestRef.id}`);

            // Verify we can read it back
            const leaveRequestsQuery = query(collection(getFirebaseDb(), 'leaveRequests'), limit(5));
        ...
        try {
            ...
            const policyRef = doc(collection(getFirebaseDb(), 'policies'), 'sync-test-policy');
            await setDoc(policyRef, testPolicy);
            console.log(`✅ Created policy: sync-test-policy`);

            // Verify we can read it back
            const policiesQuery = query(collection(getFirebaseDb(), 'policies'), limit(5));
        ...
        try {
            ...
            const notificationRef = await addDoc(collection(getFirebaseDb(), 'notifications'), testNotification);
            console.log(`✅ Created notification: ${notificationRef.id}`);

            // Verify we can read it back
            const notificationsQuery = query(collection(getFirebaseDb(), 'notifications'), limit(5));
        ...
        try {
            ...
            const goalRef = await addDoc(collection(getFirebaseDb(), 'performanceGoals'), testGoal);
            console.log(`✅ Created performance goal: ${goalRef.id}`);

            // Verify we can read it back
            const goalsQuery = query(collection(getFirebaseDb(), 'performanceGoals'), limit(5));
        ...
        try {
            ...
            const unsubscribe = onSnapshot(
                query(collection(getFirebaseDb(), 'notifications'), limit(1)),
                (snapshot) => {
...
export async function createSyncTestData() {
    ...
    try {
        ...
        await setDoc(doc(collection(getFirebaseDb(), 'employees'), 'sync-test-employee'), employeeData);
        ...
        for (const leaveType of leaveTypes) {
            await setDoc(doc(collection(getFirebaseDb(), 'leaveTypes'), leaveType.id), leaveType);
        }
