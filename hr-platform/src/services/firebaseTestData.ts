...
export async function createTestData() {
    try {
        ...
        for (const leaveType of testLeaveTypes) {
            const docRef = doc(collection(getFirebaseDb(), 'leaveTypes'), leaveType.id);
        ...
        for (const policy of testPolicies) {
            const docRef = doc(collection(getFirebaseDb(), 'policies'), policy.id);
        ...
        for (const employee of testEmployees) {
            const docRef = doc(collection(getFirebaseDb(), 'employees'), employee.id);
...
export async function testFirebaseCollections() {
    try {
        ...
        const leaveTypesQuery = query(collection(getFirebaseDb(), 'leaveTypes'), limit(5));
        ...
        // Test employees
        const employeesQuery = query(collection(getFirebaseDb(), 'employees'), limit(5));
