...
export async function getOfficeLocationService(): Promise<IOfficeLocationService> {
    ...
    try {
        ...
        if (config.firebase && config.firebase.enabled && config.firebase.db) {
            ...
            console.log('✅ Using Firebase Office Location Service (Employee)');
            return serviceInstance;
        }

        throw new Error('Firebase not properly configured');
    } catch (error) {
        console.error('❌ Error initializing Office Location Service (Employee):', error);
        throw error;
    }
}
...
export function formatDistance(distanceKm: number, radiusMeters: number = 100): string {
