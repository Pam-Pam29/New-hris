...
export class MeetingNotificationService {
    ...
    startMeetingChecker(employeeId: string, callback: (meeting: any) => void) {
        // Check every minute for meetings starting soon
        this.checkInterval = setInterval(async () => {
            await this.checkUpcomingMeetings(employeeId, callback);
        }, 60000); // Check every 1 minute

        // Also check immediately
        this.checkUpcomingMeetings(employeeId, callback);
    ...
    private async checkUpcomingMeetings(employeeId: string, callback: (meeting: any) => void) {
        try {
            const now = new Date();
            const tenMinutesFromNow = new Date(now.getTime() + 10 * 60000);

            // Query for approved meetings for this employee
            const q = query(
            ...
            querySnapshot.forEach((doc) => {
                ...
                if (meetingDate >= now && meetingDate <= tenMinutesFromNow) {
                    ...
                    if (!this.notifiedMeetings.has(meeting.id)) {
                        ...
                        callback(meeting);
                        this.sendMeetingStartNotification(meeting);
                    }
                }
            });
        } catch (error) {
    ...
    private async sendMeetingStartNotification(meeting: any) {
        try {
            const notification = {
                employeeId: meeting.employeeId,
                title: '🔔 Meeting Starting Soon!',
                message: `Your meeting "${meeting.title}" starts in a few minutes`,
                type: 'info',
                category: 'performance',
                read: false,
                actionUrl: meeting.meetingLink || '/performance-management',
