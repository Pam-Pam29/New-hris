...
import { LeaveType, LeaveRequest, LeaveBalance, normalizeLeaveStatus } from '../../../types/leaveManagement';
...
class LeaveService {
    ...
    // Submit leave request using sync service
    async submitLeaveRequest(leaveRequest: Omit<LeaveRequest, 'id' | 'submittedAt' | 'status'>): Promise<boolean> {
        try {
            console.log('📝 Submitting leave request via sync service:', leaveRequest);
            
            const requestId = await leaveSyncService.submitLeaveRequest(leaveRequest);
    ...
    // Cancel leave request using sync service
    async cancelLeaveRequest(requestId: string): Promise<boolean> {
        try {
            console.log('🚫 Cancelling leave request via sync service:', requestId);
